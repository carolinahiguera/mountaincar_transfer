...
from utils.common import tf_util, OffPolicyRLModel, SetVerbosity, TensorboardWriter
from utils.common.vec_env import VecEnv
from utils.common.schedules import LinearSchedule
from utils.common.buffers import ReplayBuffer, PrioritizedReplayBuffer
from deepq.build_graph import build_train
from deepq.policies import DQNPolicy
from deepq.dqn2 import DQN


class DQN_TRANSFER(OffPolicyRLModel):
    """
    The DQN model class.
    ...
    """
    def __init__(self, policy, env, sim, path_model_Tsource, one_classifier, Nsequence=2, gamma=0.99, learning_rate=5e-4, buffer_size=50000, exploration_fraction=0.1,
        ...
        super(DQN_TRANSFER, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose, policy_base=DQNPolicy,
        ...
        self.model_Tsource = DQN.load(path_model_Tsource,env)
    ...
    def learn(self, total_timesteps, callback=None, log_interval=100, tb_log_name="DQN",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            # Phi for transfer
            self.phi = LinearSchedule(schedule_timesteps=int(self.exploration_fraction * total_timesteps),
                                              initial_p=1.0,
                                              final_p=0.1)
            ...
            for _ in range(total_timesteps):
                ...
                if not self.param_noise:
                    ...
                    update_phi = self.phi.value(self.num_timesteps)
                ...
                # print(f'eps={update_eps}')
                # print(f'phi={update_phi}')
                ...
                if(reset):
                    if(self.Nsequence==3):
                        ...
                        env_action = action                        
                        new_obs, _, _, _ = self.env.step(env_action)
                        steps_episode +=1
                        with self.sess.as_default():
                            action = self.act(np.array(new_obs)[None], update_eps=update_eps, **kwargs)[0]
                        env_action = action
                        reset = False
                        new_obs2, _, _, _ = self.env.step(env_action)   
                        steps_episode +=1
                        sequence = np.array([obs[0], obs[1], new_obs[0], new_obs[1], new_obs2[0], new_obs2[1]])
                        obs = new_obs2
                    else:
                        with self.sess.as_default():
                            action = self.act(np.array(obs)[None], update_eps=update_eps, **kwargs)[0]
                ...
                with self.sess.as_default():
                    if(self.one_classifier.predict(sequence.reshape(1,-1))[0]==1):
                        Qk_1 = update_phi * self.model_Tsource.predict(obs)[1][0]
                        Qk = (1.0-update_phi) * self.predict(obs)[1][0]
                ...
                # reward_, done = self.compute_reward(new_obs, reward_, timesteps)
                #------------------------

                # print(f'reward={reward_}')
                ...
                if done:
                    ...
                    reset = True
                    timesteps = 0
                    record_stepsEpisode.append(steps_episode)
                    steps_episode = 0
        ...
        np.save(f'{self.tensorboard_log}/record_stepsEpisode_{self.sim}.npy',np.array(record_stepsEpisode))
        return self
